...
const PROMPT_STYLES = {
  ultra_short: {
    continuation: '[續行] 前次工作尚未完成。待辦 {n} 項。下一項：「{task}」直接執行。完成用 todowrite。不要問。',
    toolErrorL1: '[續行] 執行「{task}」時 tool 失敗。重試。直接執行。',
    toolErrorL2: '[續行] 「{task}」tool 失敗 {n} 次。換方法做。直接執行。',
    toolErrorL3: '[續行] 「{task}」tool 一直失敗。用全新方法。不要用 {lastTool}。直接執行。',
    toolErrorL4: '[續行] tool 一直失敗。用 websearch 查解法。不要猜。',
    recoveryL0: '繼續任務',
    recoveryL1: '繼續執行前次工作',
    recoveryL2: '繼續: {task}',
    forgotten: '[續行] 任務「{task}」已多次注入但 todowrite 狀態未更新。若已完成請用 todowrite 設為 completed，否則繼續執行。',
    contextPressure: '[續行] context 壓力大。任務剩 {n}。「{task}」直接完成，避免大型輸出。',
    stuck: '[續行] 卡住。換全新方法做「{task}」。不要重複。直接執行。',
  },
  short: {
    continuation: '[續行] 前次工作尚未完成。待辦還有 {n} 項。下一項：「{task}」，完成後用 todowrite 更新。',
    toolErrorL1: '[續行] 執行「{task}」時 tool 錯誤。重試。注意指令。直接執行。',
    toolErrorL2: '[續行] 「{task}」tool 錯誤 {n} 次。請改用其他方式做。不要重複。',
