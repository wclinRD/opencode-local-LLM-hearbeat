...
在專案的 `opencode.json`（或 `~/.config/opencode/opencode.json`）的 `plugin` 陣列中加入：
...
[".opencode/plugins/smart-heartbeat-local/index.js", {
  ...
  "skillHints": [
    {
      "name": "writing-plans",
      "desc": "建立結構化實作計畫 (2-5 min/task)"
    },
    {
      "name": "test-driven-development",
      "desc": "強制 RED-GREEN-REFACTOR 循環"
    },
    {
      "name": "systematic-debugging",
      "desc": "4 階段 root cause 除錯"
    }
  ]
}]
```
...
```bash
# 檢查 plugin 可載入
node -e "require('.opencode/plugins/smart-heartbeat-local/index.js')"
...
// LLM_INSTALL_INSTRUCTIONS
{
  ...
  "configTemplate": {
    "entry": ".opencode/plugins/smart-heartbeat-local/index.js",
    "options": {
      "allowAllAgents": true,
      "countdownSeconds": 30,
      "minIntervalMs": 90000,
      "maxStuckCycles": 8,
      "skillHints": [
        {"name": "writing-plans", "desc": "建立結構化實作計畫 (2-5 min/task)"},
        {"name": "test-driven-development", "desc": "強制 RED-GREEN-REFACTOR 循環"},
        {"name": "systematic-debugging", "desc": "4 階段 root cause 除錯"}
      ]
    }
  },
  "verifyCommands": [
...
| 症狀 | 原因 | 解法 |
|------|------|------|
| `require()` fail | 路徑錯誤 | 確認 `.opencode/plugins/smart-heartbeat-local/` 存在且 index.js 在該目錄 |
| `/heartbeat` 命令無回應 | plugin 未載入 | 確認 opencode.json 的 plugin entry point 正確 |
| 注入提示未出現 | event hook 未註冊 | 檢查 onStart 有註冊 `message.completed` handler |
