...
const plugin = {
  ...
  server: async (input, config = {}) => {
    ...
    async function checkAndInject(sessionID) {
      ...
      // Processing guard: timeout release (2x timer interval to avoid race)
      if (state.processingGuard) {
        const guardAge = Date.now() - (state.lastInjectionTime || 0)
        const guardTimeout = Math.max(mergedConfig.countdownSeconds * 1000 * 2, 60000)
      ...
      // In-flight tool check (all tool types, not just 'task')
      ...
      if (state.lastToolTime && Date.now() - state.lastToolTime < 30000) return
